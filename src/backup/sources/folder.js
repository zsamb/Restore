...
class folder {
    ...
    verify() {
        return new Promise((resolve, reject) => {
            ...
            .then((stats) => { if (stats.isDirectory()) { resolve() } else { reject() } })
    ...
    verifySync() {
        if (typeof this.location == "string") {
            ...
            if (exists) {
                ...
                else { throw new Error(`File path leads to a file`) }
            } else { throw new Error(`Source folder does not exist: ${this.location}`)}
        } else { throw new Error(`Source path is invalid: ${this.location ? this.location : "undefined"}`) }
    }

    read(archive) { 
        try {
            if (this.location[this.location.length - 1] == "/") { this.location[this.location.length - 1] = "" }
            let destDir = this.location.split("/")[this.location.split("/").length - 1];
            archive.directory(this.location, destDir); 
        } catch (error) { throw new Error(`Failed to compress source folder: ${error.message}`) }
    }
