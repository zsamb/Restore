...
class file {
    ...
    verify() {
        return new Promise((resolve, reject) => {
            if (typeof this.location == "string") {
                fs.promises.access(this.location, fs.constants.F_OK | fs.constants.R_OK)
                    ...
                    .then((stats) => {
                        if (!stats.isDirectory()) {
                            ...
                            reject("File path leads to a directory")
                        }
                    })
                    .catch(error => {
                ...
                reject(`Source path is invalid: ${this.location ? this.location : "undefined"}`)
            }
        })
    }
        ...
        if (typeof this.location == "string") {
            ...
            if (exists) {
                ...
                if (!stats.isDirectory()) {
                    ...
                    throw new Error(`File path leads to a directory`)
                }
            } else {
                throw new Error(`Source file does not exist: ${this.location}`)
            }
        } else {
    ...
    read(archive) {
        try {
            ...
            throw new Error(`Failed to compress source file: ${error.message}`)
        }
    }
