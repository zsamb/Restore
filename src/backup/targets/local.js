...
class local {
        ...
        return new Promise((resolve, reject) => {
            if (typeof this.location == "string" && this.location.length > 2) {
                ...
                testStream.on("error", (error) => {
                    reject(`Invalid target location, is not writable: ${this.location}`)
                })
                testStream.on("ready", () => {
                    testStream.close();
                    //Cleanup
                ...
                reject(`Invalid local target location: ${this.location}`)
            }
        })
    }
    ...
    verifyRestore() {
        return new Promise((resolve, reject) => {
            ...
            testStream.on("error", (error) => {
                reject("Could not access backup files")
            });
            testStream.on("ready", () => {
                testStream.close();
                resolve();
    ...
    createStream() {
        try {
            ...
            throw new Error("Failed create writable stream to local output")
        }
    }
    ...
    restoreStream() {
        try {
            ...
            throw new Error("Failed to create read stream to local restore target")
        }
    }
    ...
    delete() {
        return new Promise((resolve, reject) => {
            fs.promises.access(this.location, fs.constants.R_OK | fs.constants.W_OK)
                ...
                .catch(error => {
                    reject("Backup files no longer exist")
                })
        })
    }
